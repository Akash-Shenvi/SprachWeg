...
import { Check, ChevronRight, BookOpen, Clock, Target, Award, Star, Shield, Zap } from 'lucide-react';
...
const faqs = [
  ...
  },
  {
    ...
    answer: 'All study materials, practice worksheets, audio resources, and mock tests are included in the course fee.'
  }
];
...
const CourseLevelCard: React.FC<CourseLevelCardProps> = ({ level, index, isAdvanced }) => {
  const navigate = useNavigate();
  const shouldReduceMotion = useReducedMotion();

  const handleEnroll = () => {
    // Call window.enrollToCourse if available, otherwise fallback
    if (typeof (window as any).enrollToCourse === 'function') {
      (window as any).enrollToCourse(`english-${level.id}`);
    } else {
      console.warn('Enrollment function not available');
      // Fallback: could navigate to enrollment page
      navigate('/register');
    }
  };
  ...
  const handleLearnMore = () => {
    ...
    navigate(`/training/english/${level.id}`);
  };
  ...
  return (
    ...
    >
      ...
      {/* Exam Special Pill (Only for Advanced) */}
      {isAdvanced && (
        <div className="mb-6">
          <div className="rounded-xl border-2 border-dashed border-[#d6b161] bg-[#d6b161]/5 p-4">
            <div className="mb-2 flex items-center gap-2">
              <Award className="h-5 w-5 text-[#d6b161]" />
              <span className="text-sm font-bold text-gray-900 dark:text-gray-100">Exam Preparation</span>
            </div>
            <p className="mb-2 text-xs text-gray-600 dark:text-gray-400">{examSpecial.name}</p>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#0a192f] via-[#112240] to-[#1a365d]">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-24 text-center sm:px-6 sm:py-32 lg:px-8">
          ...
          >
            Business & Academic English Mastery
          </motion.h1>
          ...
          >
            Develop fluent, confident English for global workplaces, universities, and international opportunities. Master professional communication from foundation to advanced.
          </motion.p>
      ...
      <main id="main-content" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <section className="mb-20">
          ...
          >
            ...
            <p className="mx-auto max-w-2xl text-base text-gray-600 dark:text-gray-400">
              Structured programs designed for your current proficiency. Start from the basics or advance your professional communication skills.
            </p>
          </motion.div>
        ...
        >
          ...
          <h2 className="mb-4 text-2xl font-bold sm:text-3xl">Ready to Start Your Journey?</h2>
          <p className="mx-auto mb-8 max-w-2xl text-base text-gray-300">
            Join thousands of learners who have transformed their English skills. Choose your level and start today.
          </p>
          <motion.button
