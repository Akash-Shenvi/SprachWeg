    ...
    React.useEffect(() => {
        const fetchDetails = async () => {
            try {
                ...
                const courses = await skillAPI.getAll('plc');
                const course = courses.find((c: any) => c.title.toLowerCase().includes('plc') || c.title.toLowerCase().includes('programmable logic controller'));

                if (course) {
                    let details = {
                        duration: course.duration || '56 hours (7 weeks)',
                        deliveryMode: 'Online / Offline / Hybrid',
                        classTimings: 'Tue, Thu, Sat - 6 PM to 9 PM',
                        fees: '₹9200 to ₹17800'
                    };
